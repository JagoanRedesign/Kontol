...
async def toggle_welcome(c: Client, m: types.Message):
    ...
    if command == "on":
        ...
        if active_welcomes:
            ...
            await m.reply(f"**Welcome yang saat ini aktif:**\n{welcome_list}")
        else:
            await m.reply("Tidak ada welcome yang saat ini aktif.")
    else:
        await m.reply("Perintah tidak dikenali. Gunakan `.wlc on`, `.wlc off`, atau `.wlc list`.")
...
async def set_welcome(c: Client, m: types.Message):
    ...
    if not new_welcome_message:
        await m.reply("**Penggunaan:** `.set_welcome <kalimat_sambutan>`")
        return
    
    group_settings[chat_id]["welcome_message"] = new_welcome_message
    await m.reply(f"Kalimat sambutan telah diubah menjadi: `{new_welcome_message}`")
...
async def welcome_new_member(c: Client, m: types.ChatMemberUpdated):
    ...
    if chat_id in group_settings and group_settings[chat_id]["welcome_enabled"] and m.new_chat_member.status == "member":
        user_name = m.new_chat_member.user.first_name
